...
var renderValues = require('../debug/renderValues')

function nextIndex(ceil, type) {
  // set the default index to random
  type = type !== undefined ? type : 'newest'
...
function growingTree(settings, grid, cx, cy) {
  // default parameters
  ...
  var x = randomInteger(0, width - 1, settings.generator.random())
  var y = randomInteger(0, height - 1, settings.generator.random())
  cells.push([x, y])

  while(cells.length > 0) {

    // pick the next cell
    var index = nextIndex(cells.length)
